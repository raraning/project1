...
<input type="hidden" id="uid" name="uid">
	...
	<div class="viewfid">
		...
		<form action="comment_update.jsp?code=<%=code%>&uid=<%=uid%>&tb_uid=<%=tb_uid%>&c_uid=<%=c_uid%>" method="post">
		...
		String sql_comment2 = "select * from comment where tb_table='"+code+"' and tb_uid="+tb_uid2+" and uid="+c_uid2;
	...
	<div class="ftwb">
		...
		<div class="ftwbb b2 fb_button">
			<%
			if(rs.getString("id").equals(session_id)){%>
				<a href="modify.jsp?code=<%=code%>&uid=<%=uid%>">수정하기</a>
			<%}else{%>
				<a href="javascript:alert('글의 작성자 혹은 관리자만 수정이 가능합니다');">수정하기</a>
			<%}%>
		</div>
		<div class="ftwbb" style="width:10px;"></div>
		<div class="ftwbb b3 fb_button"><a href="list.jsp?code=<%=code%>">글목록</a></div>
	</div>
<div style="height:15px;"> </div>
<%}%>
