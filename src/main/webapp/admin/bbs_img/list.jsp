...
<script>
	function bbsdel(bbs_uid){
		...
		if(result){
			location.href="/admin/bbs_img/delete.jsp?code=<%=code%>&uid="+bbs_uid;
			return true;
		}
	}
</script>
...
%>
	...
	%>
	<div class='bbs_gal' style="height:270px;">
	<%
	if(gongji.equals("3")){
		if((session_nickname != null && session_nickname != "" && session_nickname.equals(nickname)) || (session_level != null && session_level != "" && session_level.equals("관리개발자"))){%>		
			...
			<button style="cursor:pointer;" onclick="bbsdel('<%=uid%>')">삭제하기</button>
		<%}else{%>
