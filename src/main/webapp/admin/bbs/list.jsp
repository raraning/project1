...
<%
if("noticecss".equals(code) || "qnacss".equals(code)){%>
	<div class="join_button1" style="display:inline-block;"><a href="/admin/bbs/list.jsp?code=noticecss">CSS지식공유 게시판 바로가기</a></div>
	<div class="join_button1" style="display:inline-block;"><a href="/admin/bbs/list.jsp?code=qnacss">CSS질문&답변 게시판 바로가기</a></div>
<%}else{%>
	<div class="join_button1" style="display:inline-block;"><a href="/admin/bbs/list.jsp?code=noticesql">MYSQL지식공유 게시판 바로가기</a></div>
	<div class="join_button1" style="display:inline-block;"><a href="/admin/bbs/list.jsp?code=qnasql">MYSQL질문&답변 게시판 바로가기</a></div>
<%}
%>
</div>
<div style="height:15px;"> </div>
...
<div class="ft_list">
	...
	<div class="ftlist ft2" style="width:500px">제목</div>
	<div class="ftlist ft3">글쓴이</div>
	<div class="ftlist ft4">작성일</div>
	<div class="ftlist ft5">조회수</div>
	<div class="ftlist ft6">삭제</div>
</div>

<script>
	function bbsdel(bbs_uid){
		
		var result = confirm("삭제하시겠습니까?");
		if(result){
			location.href="/admin/bbs/delete.jsp?code=<%=code%>&uid="+bbs_uid;
			return true;
		}
	}
</script>
...
<div class="ft_list2" style="background-color:#ffd6d5;font-weight:bold;">
	...
	<a href="view.jsp?code=<%=code%>&page_now=<%=page_now%>&uid=<%=uid%>&field=<%=field%>&search=<%=search%>"><div class="ftlist2 ft22" style="width:500px"><%=subject%></div></a>
	...
	<div class="ftlist2 ft6"><button style="cursor:pointer;" onclick="bbsdel('<%=uid%>')">삭제</button></div>
</div>
	<%
	}
}
...
<div class="ft_list2">
	...
	if(gongji.equals("3")){
		if((session_nickname != null && session_nickname != "" && session_nickname.equals(nickname)) || (session_level != null && session_level != "" && session_level.equals("관리개발자"))){%>
			...
			<div class="ftlist2 ft22"  style="width:500px">
				<%
				//답변 들여쓰기 및 아이콘 출력
				if(len_thread > 1){
					...
					out.print("<img src='img/enter.png' width=13 height=13>");
				}
				%>
...
<script>
	function alldel(){
		...
		if(result){
			location.href="/admin/bbs/alldelete.jsp?code=<%=code%>";
			return true;
		}
	}
</script>
<div class="ft_bottom">
	...
	<div class="ftbt fb2" style="width:672px">
	...
	<div class="ftbt delred"><a href="javascript:alldel();">글전체삭제</a></div>
